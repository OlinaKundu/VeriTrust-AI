...
from app.services.audio_detector import extract_audio_from_video, analyze_audio
...
async def scan_file_sync(
    ...
    try:
        ...
        result = await run_pipeline_sync(temp_path, scan_mode, file.filename)
        return result
    finally:
        cleanup_file(temp_path)
...
async def websocket_analyze(websocket: WebSocket, file_id: str, scan_mode: str = "full"):
    ...
    try:
        if scan_mode == "ela":
            ...
            await run_full_pipeline_ws(websocket, file_path)
...
async def run_pipeline_sync(file_path: Path, scan_mode: str, original_filename: str) -> Dict[str, Any]:
    ...
    if scan_mode == "ela":
        ...
        for k in keyframes:
            ...
            for face_img in k["faces"]:
                score, heatmap = analyze_face_frame(face_img)
        ...
        if audio_extracted:
            audio_details = analyze_audio(temp_audio_path)
...
async def run_full_pipeline_ws(websocket: WebSocket, file_path: Path):
    hw_info = get_cuda_device_info()
    await websocket.send_json({
    ...
    await websocket.send_json({"status": "Analyzing face boundaries using GPU tensors...", "progress": 30})
